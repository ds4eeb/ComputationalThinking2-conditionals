...
In this example, `traceback()` prints `1: print(x * 2) at #3`. `1:` tells you that this is the first thing it tried to do. The `print(x*2)` tells you what it tried to do that didn't work, and `at #3` means the error is at line #3. This can be particularly helpful if you are running a big function, loop, series of functions, etc, and need to figure out exactly when and where that pesky error started.
...
The researchers measured concentration on a continuous scale in the `concentration_pg_g` column. For `case_when()` practice, let's turn this into a categorical scale by assigning an arbitrary threshold after which we call a pika "stressed" (warning: this is not endorsed by Pika Science). 
...
Now it's your turn: There are two parts to this last section.
...
### Q2.4 Write a couple sentences describing what you want to do
